...
/** 获取分类详情 */
export function getCategoryDetail(id: string | number): Promise<Result<Category>> {
...
/** 更新分类 */
export function updateCategory(id: string | number, data: Partial<Category>): Promise<Result<void>> {
  return request.put(`/category/${id}`, data);
}

/** 删除分类（批量） */
export function deleteCategory(ids: (string | number)[]): Promise<Result<void>> {
